...
[[ -n "${TI_METHOD}" ]] || die "Usage: $0 <ti_method> <task_1|task_2>"
[[ -n "${TASK_ID}" ]] || die "Usage: $0 <ti_method> <task_1|task_2>"
...
# Ordered short names for output naming / iteration
INTEGRATED_METHODS=(
  "scvi"
  "scanvi"
  "scgen"
...
for m in "${METHODS[@]}"; do
  case "${m}" in
    tscan)     need_file "${METHOD_R_TSCAN}" ;;
    slingshot) need_file "${METHOD_R_SLINGSHOT}" ;;
    monocle3)  need_file "${METHOD_R_MONOCLE3}" ;;
    scorpius)  need_file "${METHOD_R_SCORPIUS}" ;;
  esac
done
...
find_integrated_h5ad() {
  ...
  case "${short_name}" in
    scvi)
      path="${BASE_INTEGRATION_DIR}/scvi_full_hvg_seed0/adata_scvi.h5ad"
      ;;
    scanvi)
      path="${BASE_INTEGRATION_DIR}/scanvi_full_hvg_seed0/adata_scanvi.h5ad"
      ;;
    scgen)
      path="${BASE_INTEGRATION_DIR}/scgen_full_hvg_seed0/adata_scgen.h5ad"
      ;;
    harmony)
      path="${BASE_INTEGRATION_DIR}/harmony_full_hvg_seed0/adata_harmony.h5ad"
      ;;
    fastmnn)
      path="${BASE_INTEGRATION_DIR}/fastmnn_full_hvg_seed0/adata_fastmnn_fastmnn_full_hvg_seed0.h5ad"
      ;;
    combat)
      path="${BASE_INTEGRATION_DIR}/combat_full_hvg_seed0/adata_combat.h5ad"
      ;;
    bbknn)
      path="${BASE_INTEGRATION_DIR}/bbknn_full_hvg_seed0/adata_bbknn.h5ad"
      ;;
    scanorama)
      path="${BASE_INTEGRATION_DIR}/scanorama_full_hvg_seed0/adata_scanorama.h5ad"
      ;;
    liger)
      path="${BASE_INTEGRATION_DIR}/liger_full_hvg_seed0/adata_liger_liger_full_hvg_seed0.h5ad"
      ;;
    seurat)
      path="${BASE_INTEGRATION_DIR}/seurat_full_hvg_seed0/adata_seurat_seurat_full_hvg_seed0.h5ad"
      ;;
    *)
...
run_one() {
  ...
  "${cmd[@]}" |& sed "s/^/[${TI_METHOD}|${integrated_name}] /" | tee "${log_file}"

  print_run_done "${TI_METHOD}" "${TASK_ID}" "${integrated_name}"
}

echo "============================================================"
echo "[$(ts)] [START] one-TI-method-across-integrations"
echo "  TI method        : ${TI_METHOD}"
echo "  Task ID          : ${TASK_ID}"
echo "  Task name        : ${TASK_NAME}"
...
count=0
batch_idx=1
total_items="${#INTEGRATED_METHODS[@]}"
...
for integrated_name in "${INTEGRATED_METHODS[@]}"; do
  ...
  run_one "${integrated_name}" "${batch_idx}" "${count}" "${total_items}" &
  pids+=($!)

  if (( count % PARALLEL_BATCH_SIZE == 0 )); then
    wait_jobs "batch ${batch_idx}" "${pids[@]}"
